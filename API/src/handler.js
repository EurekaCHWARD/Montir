...
exports.getDailyData = async(req, res) => {
    ...
    try {
        ...
        const [rows] = await db.promise().query('SELECT * FROM datas WHERE date BETWEEN ? AND ? AND user_id = ?', [startDate, endDate, userId]);
...
exports.editUserById = async(req, res) => {
    const { age, city, height, weight } = req.body;
    const userId = req.params.id;
    const bmiData = calculateBMI(height, weight);
    ...
    await db.promise().query(
        'UPDATE users SET age = ?, city = ?, height = ?, weight = ?, bmi = ?, bmi_underweight = ?, bmi_normal = ?, bmi_overweight = ?, bmi_obese = ? WHERE id = ?', [age, city, height, weight, bmiData.bmi, bmiData.bmi_underweight, bmiData.bmi_normal, bmiData.bmi_overweight, bmiData.bmi_obese, userId]
    );

    // Update user's data in the datas table
    await db.promise().query(
        'UPDATE datas SET age = ?, city = ?, bmi = ?  WHERE user_id = ?', [age, city, bmiData.bmi, userId]
    );
...
exports.editUserPass = async(req, res) => {
    const { username, password } = req.body;
    const userId = req.params.id;
    ...
    // Update user's information in the users table
    await db.promise().query(
        'UPDATE users SET username = ?, password = ? WHERE id = ?', [username, hashedPassword, userId]
    );
